...
#     exported image. With cloud-init off it must be done explicitly:
#     see hooks/host_finalizeImage.sh, which runs after the guest is
#     down and after build.py has grown the disk. It CANNOT be done
#     here -- at prepareImage time the qcow2 is still the ~246M
#     upstream download, and createVMFromVHD only adds the +200G
#     afterwards.
